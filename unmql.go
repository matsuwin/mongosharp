...
// NewDatabase 连接数据库
func NewDatabase(nodes, name, user, pwd string) (*mongo.Database, error) {
	...
	if user != "" {
		...
		urlString = fmt.Sprintf(format, user, url.QueryEscape(pwd), nodes, name, writeConcern, readPreference, readConcern)
	} else {
	...
	if err = client.Ping(ctx, readpref.Primary()); err != nil {
		return nil, errors.New("[ ping database is not responding ] " + err.Error())
	}
...
// Select 聚合检索
func Select(collection *mongo.Collection, limit, page int, match, sort, project interface{}, lookups ...AggregateLookup) (
